#!/usr/bin/env python3
"""prediction_pipeline_sweep_pi_vaxis.py -- RUNS ON THE PI. Targeted re-run
of prediction_pipeline_sweep_pi.py (Step 2 full sweep) to recover PER-AXIS
signed velocity error, which the original run computed in-memory but only
persisted as a scalar norm (velocity_error_mm_s). This is a COPY of the
original script with ONLY output columns added -- fit, detection, RANSAC,
T-grid, and flight population are byte-for-byte identical to the original.
See claude/prompts/2026-08-05_1254_pi_sweep_rerun_velocity_error_components.md
and claude/claude_logs/2026-08-04_1906_pi_prediction_pipeline_sweep_worklog.md.

ONLY CHANGES vs the original prediction_pipeline_sweep_pi.py:
  1. process_flight() now also computes and persists, per (flight,T):
     vx_own/vy_own/vz_own, vx_ref/vy_ref/vz_ref (raw per-axis velocity,
     world frame: X=depth, Y=width, Z=up), and the SIGNED per-axis errors
     err_vx/err_vy/err_vz = own - ref. velocity_error_mm_s (the scalar
     norm) is kept, computed identically, for a regression check against
     the original run.
  2. Default/expected --out is a NEW filename
     (pipeline_sweep_full_vaxis_20260805.json) -- never the original
     pipeline_sweep_full_20260804.json.

Everything else below is unchanged from the original (see that file's own
docstring for the full two-clock/concurrent-with-capture model rationale).

Usage (on the Pi, inside the venv):
    ~/benchmark/venv/bin/python3 prediction_pipeline_sweep_pi_vaxis.py \
        --out pipeline_sweep_full_vaxis_20260805.json [--pilot N]
"""
import argparse
import csv
import json
...
def process_flight(session, flight_id, cfg, pooled_k, ref_row, geometries):
    ...
    # world-frame axis unit vectors, needed to project the raw camera-frame
    # velocity vector (predict_fn/fit_fn/g_fixed all operate in the same
    # camera-metric frame as the triangulated xyz, NOT pre-rotated into
    # world-semantic axes -- confirmed by reading g_fixed_for/load_g_fixed's
    # construction: g_fixed = G_MAGNITUDE*(-up_vec), up_vec itself a
    # camera-frame unit vector from the world_transform.npz) onto
    # X_world (depth) / Y_world (width) / Z_world (up). build_geometry()
    # only keeps X_world and up=Z_world in its geo dict (Y_world is loaded
    # internally by load_world_axes() but not propagated) -- load it
    # directly via the SAME frozen load_world_axes() rather than
    # reconstructing via cross product (avoids an unverified assumption
    # about handedness/sign that a cross-product guess would introduce).
    X_world_unit = X_world / np.linalg.norm(X_world)
    Z_world = geo["up"] / np.linalg.norm(geo["up"])
    _, Y_world_raw, _ = load_world_axes(session, reg)
    Y_world = Y_world_raw / np.linalg.norm(Y_world_raw)
    ...
    for T in T_VALUES_MS:
        ...
        vel_ref = ref_row["crossing_vel_xyz"]
        velocity_error_mm_s = float(np.linalg.norm(vel_own - vel_ref))
        hit_miss_match = (cls_own == ref_row["cls"])

        # -- NEW: per-axis signed velocity (world frame: X=depth, Y=width, Z=up) --
        vx_own = float(np.dot(vel_own, X_world_unit))
        vy_own = float(np.dot(vel_own, Y_world))
        vz_own = float(np.dot(vel_own, Z_world))
        vx_ref = float(np.dot(vel_ref, X_world_unit))
        vy_ref = float(np.dot(vel_ref, Y_world))
        vz_ref = float(np.dot(vel_ref, Z_world))
        ...
        row.update(
            ...
            # -- NEW columns (this script only) --
            vx_own=vx_own, vy_own=vy_own, vz_own=vz_own,
            vx_ref=vx_ref, vy_ref=vy_ref, vz_ref=vz_ref,
            err_vx=vx_own - vx_ref, err_vy=vy_own - vy_ref, err_vz=vz_own - vz_ref,
        )
        result["t_rows"].append(row)

...
def process_flight(session, flight_id, cfg, pooled_k, ref_row, geometries, arm):
    ...
    # -- FROZEN DETECTIONS. detect_flight_threaded() is NOT called. -----------
    # The original writes det0/det1 to these two per-camera CSVs and immediately
    # feeds those same paths into build_corrected_pairs. Reading them back enters
    # the pipeline at exactly the point the detect call exited: same two CSVs,
    # same timestamps file, same arguments. Everything from build_corrected_pairs
    # onward - trajectory filter, pairing, triangulation, fit - is untouched.
    #
    # Detection is deterministic given the same images and config, so this is a
    # substitution of a fixed function's cached output, not a fidelity change.
    # The 60 fps regression check against pipeline_sweep_raw.csv is what proves
    # that; it is load-bearing, not a formality.
    over_cadence = []          # a detection-timing artefact; not produced here
    # Empty on purpose. It fed only last_pair_detect_ms -> latency_ms, which are
    # TIMING fields and void for this run. Leaving it empty makes them NaN rather
    # than inventing a laptop number that could later be mistaken for a Pi one.
    t_pair_ms = {}
    ...
    for p_ in (c0_csv, c1_csv):
        if not p_.is_file():
            result["status"] = "skipped"
            result["reason"] = f"frozen detections missing: {p_.name}"
            return result
    ...
    # ---- DECIMATION -------------------------------------------------------
    # Applied on CAM0 FRAME NUMBER, not on position in the pair list. A camera
    # running at 30 fps captures every 2nd frame whether or not the detector
    # found the ball in it, so a detection gap must not shift the phase of every
    # later sample. Selecting on frame % step == offset reproduces the physical
    # camera; selecting on list index would not.
    #
    # This happens BEFORE the window is cut, so a window of a given DURATION
    # holds proportionally fewer points. It is not a shortening of the window.
    step, offset = arm["step"], arm["offset"]
    keep = [i for i, p in enumerate(pairs) if p["cam0_frame"] % step == offset]
    if len(keep) < MIN_SAMPLES_C:
        result["status"] = "skipped"
        result["reason"] = (f"only {len(keep)} pairs survive decimation "
                            f"step={step} offset={offset} (< min_samples={MIN_SAMPLES_C})")
        return result
    ...
    # Re-anchored on the FIRST RETAINED pair: a camera at this rate and phase
    # starts its clock at its own first sample. At step=1, offset=0 this reduces
    # exactly to the original anchor, which is what keeps the 60 fps arm
    # bit-identical to the existing sweep.
    ...
    for T in T_VALUES_MS:
        row = {"T_ms": T, "fps": arm["fps"], "phase": arm["offset"]}
...
def main():
    ...
    ap.add_argument("--rates", default="60",
                    help="comma list of fps arms to run, or 'all'. Default 60 only, "
                         "so the regression arm can be run first and alone.")
    ap.add_argument("--phases", default=None,
                    help="comma list of phase offsets to keep, within the selected "
                         "rates. Lets one arm run as its own process so a crash "
                         "costs one arm, not the whole run. Selection only; the "
                         "computation for a given (fps, phase) is unchanged.")
    ...
    # Decimation arms. 60 fps keeps every frame (1 phase); 30 fps keeps every
    # 2nd (2 phases); 20 fps every 3rd (3 phases). Phase is the frame-number
    # offset, so each arm covers every possible alignment of that camera rate.
    ALL_ARMS = [
        {"fps": 60, "step": 1, "offset": 0},
        {"fps": 30, "step": 2, "offset": 0},
        {"fps": 30, "step": 2, "offset": 1},
        {"fps": 20, "step": 3, "offset": 0},
        {"fps": 20, "step": 3, "offset": 1},
        {"fps": 20, "step": 3, "offset": 2},
    ]
    if args.rates.strip().lower() == "all":
        arms = ALL_ARMS
    else:
        want = {int(x) for x in args.rates.split(",") if x.strip()}
        arms = [a for a in ALL_ARMS if a["fps"] in want]
    if args.phases is not None:
        keep = {int(x) for x in args.phases.split(",") if x.strip()}
        arms = [a for a in arms if a["offset"] in keep]
    if not arms:
        raise SystemExit(f"--rates {args.rates!r} --phases {args.phases!r} selected no arms")
    print(f"arms to run: " + ", ".join(f"{a['fps']}fps/phase{a['offset']}" for a in arms))

    results = []
    drift_samples = []
    t_start = time.time()
    for arm in arms:
        print(f"\n=== ARM {arm['fps']} fps, phase {arm['offset']} "
              f"(step {arm['step']}) ===", flush=True)
        for i, (session, flight) in enumerate(flight_list, 1):
            r = process_flight(session, flight, cfg, pooled_k,
                               reference[(session, flight)], geometries, arm)
            results.append(r)
            if i % 25 == 0 or i == len(flight_list):
                elapsed = time.time() - t_start
                print(f"  {i}/{len(flight_list)} flights done "
    ...
    if args.csv:
        ...
        cols = ["session", "flight", "fps", "phase", "T_ms", "status",
                "airborne", "n_detected", "n_points_used",
        ...
        with open(csv_path, "w", newline="") as f:
            ...
            for fl in results:
                for row in fl.get("t_rows", []):
                    ...
                    for c in cols[2:]:
                        rec[c] = row.get(c)
                    rec["fps"] = fl.get("fps", row.get("fps"))
                    rec["phase"] = fl.get("phase", row.get("phase"))
                    w.writerow(rec)
                    n += 1

# label_registration_points.py
# Manual world-frame registration point labelling: one click per reference
# point (P1..P10, fixed order from reference_points.txt), per camera. Zoom/
# pan/coordinate-inversion mechanics adapted from
# src/image_processing/03_manual_centroid_labelling/01_label_frames.py (no
# padding here - see below). Coordinates are original-image pixel space
# (OpenCV convention: origin top-left, x right, y down), stored as floats.
#
# Usage:
#   python label_registration_points.py [--cam cam0|cam1]
#   (run once per camera - default cam0, then re-run with --cam cam1)
#
# Keys:
#   s / Enter        accept current point's click, advance to next point
#   r / Delete        clear current point's click (or hide its stored marker
#                      to re-click it)
#   ← →               navigate freely between points P1..P10
...
def load_point_ids(path: Path) -> list:
    """Parse 'id x y z' rows (skip blanks / '#' comments); return ids in file order."""
    ids = []
...
def save_csv(path: Path, points: dict, point_ids: list) -> None:
    """Rewrite entire CSV in reference-point order (crash-safe: always complete)."""
...
def main():
    ap = argparse.ArgumentParser(description="Manual world-registration point labelling tool.")
    ap.add_argument("--cam", choices=["cam0", "cam1"], default="cam0",
                    help="Camera to label (default: cam0). Run again with --cam cam1 for the other.")
    args = ap.parse_args()

    cam = args.cam
    img_path = SESSION_DIR / cam / "img_0001.png"
    csv_path = SESSION_DIR / f"{cam}_points.csv"

    if not REFERENCE_POINTS_FILE.is_file():
        print(f"Reference points file not found: {REFERENCE_POINTS_FILE}")
        return
    point_ids = load_point_ids(REFERENCE_POINTS_FILE)
    if not point_ids:
        print(f"No point IDs parsed from {REFERENCE_POINTS_FILE}")
        return
    print(f"Loaded {len(point_ids)} reference point IDs: {', '.join(point_ids)}")
    ...
    WIN = f"World Registration Labeller - {cam}"

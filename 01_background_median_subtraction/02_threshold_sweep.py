# threshold_sweep.py
# Visualise ball detection across multiple DIFF_THRESHOLD values.
# Produces one 2×6 grid image per static frame in tuning/threshold_sweep/.
#
# Run from anywhere:
#   python path/to/code/threshold_sweep.py
...
OUT_DIR = SESSION / "tuning" / "08_threshold_sweep5"
OUT_DIR.mkdir(parents=True, exist_ok=True)

# ---- sweep parameters ----
THRESHOLDS = [15, 20, 25, 30, 35, 40]
PANEL_W    = 400   # px per panel → (1 ref + N thresholds) × 400 px wide

# ---- fixed detection parameters (identical to detect_static.py) ----
MIN_AREA     = 200
MAX_AREA     = 50000
MIN_CIRC     = 0.3
OPEN_KERNEL  = 7
...
print(f"Processing {len(static_paths)} images × {len(THRESHOLDS)} thresholds ...\n")

for path in static_paths:
    name    = path.stem
    img     = cv2.imread(str(path), cv2.IMREAD_GRAYSCALE)
    diff    = cv2.absdiff(img, background)          # computed once, reused per threshold
    ...
    for thresh in THRESHOLDS:
        candidates, best, mask = run_detection(diff, thresh)
        ...
        if candidates:
            ...
            print(f"  {name}  thresh={thresh:2d}:  u={u:.1f} v={v:.1f} "
                  f"a={int(area)} c={circ:.2f} n={len(candidates)}")
        else:
            put_text(dp, "NO DETECTION", y=50, color=(0, 0, 255))
            print(f"  {name}  thresh={thresh:2d}:  NO DETECTION")
